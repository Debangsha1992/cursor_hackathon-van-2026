...
describe("botRuleEngine - BOT_NO_STOP_LOSS", () => {
  ...
  it("does not flag BOT_NO_STOP_LOSS when stopLoss is set", () => {
    ...
    expect(violations.find((v) => v.code === "BOT_NO_STOP_LOSS")).toBeUndefined();
  });
});
...
describe("botRuleEngine - BOT_STRATEGY_MISMATCH", () => {
  it("flags a trade whose strategyType differs from the profile (non-custom)", () => {
    ...
    const profile: BotTradingProfile = { ...baseProfile, strategyType: "trend_following" };
    const violations = detectBotRuleViolations(trade, profile, [], []);
    expect(violations.find((v) => v.code === "BOT_STRATEGY_MISMATCH")).toBeDefined();
  });

  it("does not flag BOT_STRATEGY_MISMATCH when strategyType matches", () => {
    const violations = detectBotRuleViolations(baseTrade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_STRATEGY_MISMATCH")).toBeUndefined();
  });
...
describe("botRuleEngine - BOT_POOR_RISK_REWARD", () => {
  it("flags a trade where reward-to-risk ratio is below 1.5", () => {
    const trade: BotPaperTrade = { ...baseTrade, entryPrice: 65000, stopLoss: 64000, takeProfit: 65500 };
    const violations = detectBotRuleViolations(trade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_POOR_RISK_REWARD")).toBeDefined();
  });

  it("does not flag BOT_POOR_RISK_REWARD when R:R >= 1.5", () => {
    const trade: BotPaperTrade = { ...baseTrade, entryPrice: 65000, stopLoss: 64000, takeProfit: 67000 };
    const violations = detectBotRuleViolations(trade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_POOR_RISK_REWARD")).toBeUndefined();
  });
...
describe("botRuleEngine - BOT_OVERTRADING", () => {
  it("flags a trade when recentTrades.length >= profile.maxTradesPerDay", () => {
    const recent: BotPaperTrade[] = Array.from({ length: baseProfile.maxTradesPerDay }, () => baseTrade);
    const violations = detectBotRuleViolations(baseTrade, baseProfile, recent, []);
  ...
  it("does not flag BOT_OVERTRADING when below the daily cap", () => {
    const recent: BotPaperTrade[] = Array.from({ length: baseProfile.maxTradesPerDay - 1 }, () => baseTrade);
    const violations = detectBotRuleViolations(baseTrade, baseProfile, recent, []);
    expect(violations.find((v) => v.code === "BOT_OVERTRADING")).toBeUndefined();
  });
});
...
describe("botRuleEngine - BOT_OVERCONFIDENCE", () => {
  ...
  it("does not flag BOT_OVERCONFIDENCE at 0.85", () => {
    ...
    const violations = detectBotRuleViolations(trade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_OVERCONFIDENCE")).toBeUndefined();
  });
});
...
describe("botRuleEngine - BOT_INVALID_CONFIDENCE", () => {
  it("flags a trade with confidenceScore below 0", () => {
    ...
    const violations = detectBotRuleViolations(trade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_INVALID_CONFIDENCE")).toBeDefined();
  });
  ...
  it("does not flag BOT_INVALID_CONFIDENCE when confidence is in [0,1]", () => {
    ...
    const violations = detectBotRuleViolations(trade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_INVALID_CONFIDENCE")).toBeUndefined();
  });
});
...
describe("botRuleEngine - BOT_MISSING_REASONING", () => {
  ...
  it("flags a trade whose signalReason is shorter than 30 chars", () => {
    ...
    const violations = detectBotRuleViolations(trade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_MISSING_REASONING")).toBeDefined();
  });

  it("does not flag BOT_MISSING_REASONING when signalReason is substantive", () => {
    const violations = detectBotRuleViolations(baseTrade, baseProfile, [], []);
    expect(violations.find((v) => v.code === "BOT_MISSING_REASONING")).toBeUndefined();
  });
});
