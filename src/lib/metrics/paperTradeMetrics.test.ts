...
describe("paperTradeMetrics - shape", () => {
  it("computes a single-trade sparkline and current score", () => {
    ...
    expect(result.topViolationCodes[0]).toEqual({ code: "BOT_NO_STOP_LOSS", count: 1 });
  });

  it("orders top violation codes by frequency, capped at 3", () => {
    const trades: ScoredTrade[] = [
      ...Array.from({ length: 10 }, () => ({ score: 60, violations: [{ code: "BOT_NO_STOP_LOSS" }] })),
      ...Array.from({ length: 7 }, () => ({ score: 70, violations: [{ code: "BOT_OVERCONFIDENCE" }] })),
      ...Array.from({ length: 4 }, () => ({ score: 80, violations: [{ code: "BOT_POOR_RISK_REWARD" }] })),
      ...Array.from({ length: 2 }, () => ({ score: 90, violations: [{ code: "BOT_OVERTRADING" }] })),
